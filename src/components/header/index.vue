 ...
 * @LastEditTime: 2025-05-15 15:58:16
        ...
        <div class="info-right">
            <div class="tool">
                <div @click="eleMapClick">
                    <el-tooltip popper-class="tip-popper" class="box-item" effect="dark" content="加载vtpk" placement="bottom">
                        <Icon :color="eleMap ? '#7afafe' : '#fff'" :font-size="25" icon="tool-bar-ememap" />
                    </el-tooltip>
                </div>
                <div @click="TechnologyClick">
                    <el-tooltip popper-class="tip-popper" class="box-item" effect="dark" content="科技" placement="bottom">
                        <Icon :color="Technologyshow ? '#7afafe' : '#fff'" :font-size="25" icon="tool-bar-keji" />
                    </el-tooltip>
                </div>
                <div @click="UIClick">
                    <el-tooltip class="box-item" effect="dark" content="UI显隐" placement="bottom">
                        <Icon :color="!UIShow ? '#7afafe' : '#fff'" :font-size="26" icon="mianban" />
                    </el-tooltip>
                </div>
                <div @click="LayerTreeClick">
                    <el-tooltip popper-class="tip-popper" class="box-item" effect="dark" content="图层树" placement="bottom">
                        <Icon :color="LayerTreeshow ? '#7afafe' : '#fff'" :font-size="30" icon="tucengshu" />
                    </el-tooltip>
                </div>
                <div @click="AnimationClick">
                    <el-tooltip popper-class="tip-popper" class="box-item" effect="dark" content="导览" placement="bottom">
                        <Icon :color="AnimationShow ? '#7afafe' : '#fff'" :font-size="26" icon="xunimanyou" />
                    </el-tooltip>
                </div>

                <div @click="WeatherClick">
                    <el-tooltip popper-class="tip-popper" class="box-item" effect="dark" content="气象" placement="bottom">
                        <Icon :color="WeatherShow ? '#7afafe' : '#fff'" :font-size="30" icon="qixiangjiance" />
                    </el-tooltip>
                </div>
                <!-- 工具栏 -->
                <div @click="toolBarShowClick" class="too-bar-style">
                    <el-tooltip popper-class="tip-popper" class="box-item" effect="dark" content="工具栏" placement="bottom">
                        <Icon :color="ToolBarShow ? '#7afafe' : '#fff'" :font-size="24" icon="tool-bar" />
                    </el-tooltip>
                </div>
            </div>
        </div>
...
const eleMapClick = async () => {
    ...
    if (eleMap.value) {
        ...
        vtpk = res.infotree.filter((item: any) => item.name == '矢量瓦片_广州')[0].iD
...
const TechnologyClick = async () => {
    ...
    if (Technologyshow.value) {
        ...
        __g.camera.set(430241.658257, 2555518.182109, 542.386953, -4.551392, -90.668999, 1)
    } else {
